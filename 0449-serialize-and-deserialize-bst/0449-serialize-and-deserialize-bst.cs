...
public class Codec 
{
    ...
    private void Load(TreeNode node, int level)
    {
        ...
        map[level].Add((node.val, node.left == null ? -1 : node.left.val, node.right == null ? -1 : node.right.val));
    ...
    private void ConstructTree(TreeNode node, int level)
    {
        ...
        if(left != -1) node.left = new TreeNode(left);
        if(right != -1) node.right = new TreeNode(right);
